...
| Context menu actions | `Views/ClipboardManagerView.swift` | implemented | WinUI card context menu supports `Copy` (without direct paste), `Edit` (textual items only), `Save to File`, `Copy as Base64`, `URL Encode`, `URL Decode`, plus core state actions (`Pin/Unpin`, `Delete`, `Clear History`). |
...
| Quick actions: text transformations | `Utilities/TextTransformations.swift` | partial | Windows quick-actions subset is available from context menu (`Edit` for textual items, `Copy as Base64`, `URL Encode`, `URL Decode`); broader transformation catalog parity is still pending. |
